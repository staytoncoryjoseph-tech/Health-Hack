...
async function fetchWeatherUV(lat, lon){
  // current + today UV max (daily), plus a couple extras
  const url =
...
async function fetchAirQuality(lat, lon){
  // AQI + pollutants + pollen (pollen availability depends on region/season per Open-Meteo docs)
  const hourlyVars = [
    "us_aqi",
    "pm2_5","pm10",
...
function nearestHourIndex(times){
  // times is array of ISO strings in local tz
  const now = Date.now();
  let best = 0;
...
function aqiLabel(aqi){
  ...
  if(v <= 150) return { label:`${v} (Unhealthy for Sensitive)`, cls:"warn" };
...
function pollenSummary(obj, idx){
  // Returns short allergy summary from available pollen fields
  ...
  if(!items.length){
    return "Allergies/Pollen: Not available for this region/season.";
  }

  // sort highest first
  items.sort((a,b) => Number(b[1]) - Number(a[1]));
  const top = items.slice(0,3).map(([name,val]) => `${name}: ${fmt(val,1)}`).join(" • ");
  return `Allergies/Pollen (forecast index): ${top}`;
}
...
async function loadEnvironment(lat, lon, labelText){
  ...
  // UV (daily max) + also hourly UV from air quality if available
  const uvMax = w?.daily?.uv_index_max?.[0] ?? null;

  // AIR QUALITY (nearest hour)
  ...
  if(usAqi !== null && !Number.isNaN(Number(usAqi))){
    ...
    else recs.push("Air quality looks okay: outdoor walk is a good prevention move.");
  }

  // wind/rain
  if(Number(wind) >= 20) recs.push("Windy: eye protection can help if you’re allergy-prone.");
  if(Number(current?.precipitation) > 0) recs.push("Wet conditions: road safety + layering for warmth.");
...
function larryBoost(author){
  // Original motivational text inspired by the thinker (NOT direct quotes)
  const base = {
    jung: [
  ...
  // rotate by daily hash so it changes each day
...
(function bootLarry(){
  updateSavedCount();
  // Set daily theme preview
  const author = pickDailyAuthor();
  $("todayTheme").textContent = themeFor(author);
})();
